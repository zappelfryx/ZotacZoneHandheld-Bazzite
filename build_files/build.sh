...
# CoolerControl (Offiziell – GitLab)
CC_DIR="/var/opt/coolercontrol"
mkdir -p "$CC_DIR"

CC_DOWNLOAD_URL=$(curl -sf \
    "https://gitlab.com/api/v4/projects/coolercontrol%2Fcoolercontrol/releases" \
    | grep -o '"url":"[^"]*CoolerControlD[^"]*x86_64[^"]*\.AppImage[^"]*"' \
    | head -n 1 | cut -d '"' -f 4)
...
if [ -n "$CC_DOWNLOAD_URL" ]; then
    ...
    echo "[+] CoolerControl (offiziell) heruntergeladen: $CC_DOWNLOAD_URL"
else
    echo "WARN: CoolerControl AppImage nicht gefunden – Fan-UI nicht verfügbar"
fi
...
# Decky wird bei erstem Login via firstboot-Skript installiert
DECKY_FIRSTBOOT="/usr/bin/zotac-decky-install.sh"

cat > "$DECKY_FIRSTBOOT" << 'INNEREOF'
#!/usr/bin/env bash
set -e
...
if [ -n "$DECKY_RELEASE_URL" ]; then
    ...
    echo "[+] Decky Loader installiert."
else
    echo "WARN: Decky Loader Release-URL nicht gefunden"
fi
INNEREOF
chmod +x "$DECKY_FIRSTBOOT"
...
ConditionPathExists=/home/deck/homebrew/services/PluginLoader

[Service]
Type=simple
User=deck
Environment=DECKY_HOME=/home/deck/homebrew
ExecStartPre=/usr/bin/zotac-decky-install.sh
