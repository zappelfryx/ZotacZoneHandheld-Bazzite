...
# CoolerControl (CUexter Fork – Handheld-optimiert)
CC_DIR="/var/opt/coolercontrol"
mkdir -p "$CC_DIR"

CC_DOWNLOAD_URL=$(curl -sf \
    "https://api.github.com/repos/CUexter/coolercontrol/releases/latest" \
    | grep 'browser_download_url.*CoolerControlD.*x86_64.*AppImage' \
    | cut -d '"' -f 4 | head -n 1)
...
if [ -n "$CC_DOWNLOAD_URL" ]; then
    ...
    echo "[+] CoolerControl (CUexter) heruntergeladen: $CC_DOWNLOAD_URL"
else
...
[Unit]
Description=CoolerControl Daemon (Fan Control) – CUexter Fork
After=network.target
